...
public class ShoppingCart {
	...
	@JsonProperty("itemArray")  // Add this annotation to map with client's itemArray
	...
	public int getComputedSubtotal() {
		return items.stream()
				.mapToInt(item -> item.getQuantity() * (item.getBookForm().getPrice() * 100)) // Convert to cents
				.sum();
	}
}